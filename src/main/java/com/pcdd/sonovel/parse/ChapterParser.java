...
public class ChapterParser extends Source {
    ...
    public Chapter parse(Chapter chapter) {
        ...
        // 正文内容已存在
        if (StrUtil.isNotEmpty(chapter.getContent())) {
            return chapter;
        }

        Rule.Chapter rule = this.rule.getChapter();
        Document document = jsoup(chapter.getUrl())
                .timeout(rule.getTimeout())
                .get();

        String title = JsoupUtils.selectAndInvokeJs(document, rule.getTitle());
        String content = crawl(chapter.getUrl(), rule.getInterval());
    ...
    public Chapter parse(Chapter chapter, CountDownLatch latch, SearchResult sr) {
        try {
            ...
            // 确保简繁互转最后调用
            // 处理语言设置，防止空指针异常
            String sourceLang = this.rule.getLanguage() != null ? this.rule.getLanguage() : "zh_CN";
            String targetLang = config.getLanguage() != null ? config.getLanguage() : "zh_CN";
            return ChineseConverter.convert(chapterConverter.convert(chapter), sourceLang, targetLang);
            ...
            String sourceLang = this.rule.getLanguage() != null ? this.rule.getLanguage() : "zh_CN";
            String targetLang = config.getLanguage() != null ? config.getLanguage() : "zh_CN";
            return retryChapter == null ? null : ChineseConverter.convert(retryChapter, sourceLang, targetLang);
        } finally {
            latch.countDown();
        }
    }
